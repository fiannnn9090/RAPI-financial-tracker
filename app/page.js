...
import { buildRecap, fmtShort } from '../lib/recap';
...
function Dashboard({ user, data, setData, autoGenerated = 0 }) {
  ...
  const recap = useMemo(() => buildRecap({ transactions, period: recapPeriod, streak: user.streakCurrent ?? 0 }), [transactions, recapPeriod, user.streakCurrent]);
  ...
  return <main className="dashboard-page">
      ...
      <section className="recap-section brutal-section">
        ...
        {recap.isEmpty ? <article className="brutal-card recap-card recap-empty">Belum ada cerita untuk periode ini. Catat transaksinya dulu, bestie ✨</article> : <article className="brutal-card recap-card">
          ...
          <div className="recap-stats">
            <div><small>Masuk</small><b className="in">{fmtShort(recap.stats.income)}</b></div>
            <div><small>Keluar</small><b className="out">{fmtShort(recap.stats.expense)}</b></div>
    ...
    {importSheetOpen && <ImportSheet preview={importPreview} busy={importBusy} onClose={() => { setImportSheetOpen(false); setImportPreview(null); }} onFile={handleImportFile} onReset={() => setImportPreview(null)} onConfirm={confirmImport} />}
...
function ImportSheet({ preview, busy, onClose, onFile, onReset, onConfirm }) {
  ...
  return <div className="modal-backdrop clay-modal brutal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal clay-card brutal-sheet" role="dialog" aria-modal="true" aria-labelledby="import-title" onMouseDown={(event) => event.stopPropagation()}><button className="close-modal" onClick={onClose} aria-label="Tutup">×</button><p className="kicker">IMPORT CSV</p><h2 id="import-title">Pindahkan catatanmu</h2>
    {!preview ? <>
      ...
      {sample.length > 0 && <table className="import-sample"><thead><tr><th>Tanggal</th><th>Judul</th><th>Nominal</th></tr></thead><tbody>{sample.map((row, i) => <tr key={i}><td>{row.date}</td><td>{row.title}</td><td className={row.type === 'income' ? 'in' : 'out'}>{rupiah.format(row.amount)}</td></tr>)}</tbody></table>}
