...
export default function Home() {
  ...
  return activeUser
    ? <Dashboard user={activeUser} data={data} setData={setData} autoGenerated={autoGenerated} fontScale={fontScale} onChangeFontScale={changeFontScale} />
...
function Dashboard({ user, data, setData, autoGenerated = 0, fontScale = '1', onChangeFontScale }) {
  ...
  }, [tab, showForm, budgetSheet, goalSheet, categorySheet, recurringSheet, importSheetOpen, levelUp]);
  ...
  const recap = useMemo(() => buildRecap({ transactions, period: recapPeriod, streak: user.streakCurrent ?? 0, money }), [transactions, recapPeriod, user.streakCurrent, money]);
  ...
  async function handleExportPdf() {
    ...
    if (!range) { setToast('Lengkapi rentang custom dulu ya 📅'); return; }
    const scoped = transactions.filter((tx) => tx.date >= range.start && tx.date <= range.end);
    if (!scoped.length) { setToast('Tidak ada transaksi di rentang ini 🤷'); return; }
    const label = pdfRange === 'this' ? 'Bulan Ini' : pdfRange === 'last' ? 'Bulan Lalu' : `${range.start} s/d ${range.end}`;
    try {
      await exportPdf({ transactions: scoped, rangeLabel: label, fileName: `rapi-laporan-${range.start}` });
      setToast('Laporan PDF terunduh 📄');
    } catch { setToast('Gagal bikin PDF 😵 coba lagi'); }
  }
  function handleExportCsv() {
    if (!transactions.length) { setToast('Belum ada transaksi buat diekspor'); return; }
  ...
  async function handleImportFile(event) {
    ...
    event.target.value = '';
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { setToast('File kegedean (maksimal 5MB)'); return; }
    let text;
    try { text = await file.text(); } catch { setToast('File nggak bisa dibaca 😵'); return; }
    setImportPreview({ fileName: file.name, result: csvToTransactions(text, new Set(transactions.map(dupKeyOf))) });
  }
  async function confirmImport() {
    ...
    try {
      ...
      setToast(`${inserted.length} transaksi diimpor ✨`);
    } catch {
      setToast('Import gagal di tengah jalan 😵 baris yang sudah masuk tidak diulang');
    } finally {
      setImportBusy(false);
    }
  }
  ...
  return <main className="dashboard-page">
    <div className="dashboard app-frame" id="dashboard">
      ...
      <section className="summary-grid">
        <BalanceCard balance={balance} hidden={hideBalance} money={money} onToggleHidden={toggleHideBalance} xp={user.xp ?? 0} streak={user.streakCurrent ?? 0} />
        <StatCard label="Pemasukan" amount={income} icon="↓" variant="income" hidden={hideBalance} money={money} />
        <StatCard label="Pengeluaran" amount={expense} icon="↑" variant="expense" hidden={hideBalance} money={money} />
      </section>
      <section className="recap-section brutal-section">
        <div className="section-header"><div><p className="kicker">RECAP CERITA</p><h2>Cerita keuanganmu</h2></div><div className="sort-toggle recap-toggle" role="group" aria-label="Pilih periode recap"><button type="button" className={recapPeriod === 'week' ? 'active' : ''} onClick={() => setRecapPeriod('week')}>Minggu</button><button type="button" className={recapPeriod === 'month' ? 'active' : ''} onClick={() => setRecapPeriod('month')}>Bulan</button></div></div>
        {recap.isEmpty ? <article className="brutal-card recap-card recap-empty">Belum ada cerita untuk periode ini. Catat transaksinya dulu, bestie ✨</article> : <article className="brutal-card recap-card">
          {recap.lines.map((line, i) => <p key={i}><Highlighted text={line} /></p>)}
          <div className="recap-stats">
            <div><small>Masuk</small><b className="in">{money.formatShort(recap.stats.income)}</b></div>
            <div><small>Keluar</small><b className="out">{money.formatShort(recap.stats.expense)}</b></div>
            <div><small>Hari aktif</small><b>{recap.stats.activeDays} hari</b></div>
          </div>
        </article>}
      </section>
      </>}
      ...
      <section className="manage-settings brutal-section">
        <div className="section-header"><div><p className="kicker">PENGATURAN</p><h2>Tampilan</h2></div></div>
        <article className="brutal-card settings-card">
          <label className="settings-field"><span>Mata uang tampilan</span>
            <select value={fxCode} onChange={(e) => changeCurrency(e.target.value)} aria-label="Pilih mata uang tampilan">
              {CURRENCIES.map(([code, label]) => <option key={code} value={code}>{label}</option>)}
            </select>
          </label>
          <p className="fx-note">{fxStatus === 'loading' ? 'Mengambil kurs terbaru…' : fxStatus === 'unavailable' ? 'Kurs nggak tersedia — tampilan tetap Rupiah' : fxData?.date ? `Kurs per ${fxData.date}${fxStatus === 'stale' ? ' (tersimpan)' : ''}` : 'Semua catatan tetap tersimpan dalam Rupiah.'}</p>
          <div className="settings-field" role="group" aria-label="Ukuran teks"><span>Ukuran teks</span>
            <div className="sort-toggle font-scale">
              {[['1', 'Normal'], ['1.15', 'Besar'], ['1.3', 'Super']].map(([value, label]) => (
                <button key={value} type="button" className={fontScale === value ? 'active' : ''} aria-pressed={fontScale === value} onClick={() => onChangeFontScale(value)}>{label}</button>
              ))}
            </div>
          </div>
        </article>
        ...
        <article className="brutal-card recurring-manage-card">
          {recurrings.length ? recurrings.map((rule) => <div className="recurring-row" key={rule.id}>
            ...
            <div className="recurring-info">
              ...
              <small>{money.format(rule.amount)} · {rule.frequency === 'monthly' ? `tiap bulan tgl ${rule.dayOfPeriod}` : `tiap ${WEEKDAY_LABELS[rule.dayOfPeriod] ?? 'mingguan'}`}</small>
      ...
      <section className="manage-data brutal-section">
        <div className="section-header"><div><p className="kicker">DATA & BACKUP</p><h2>Ekspor & impor</h2></div></div>
        <article className="brutal-card data-card">
          <p className="data-note">Simpan laporan PDF sesuai rentang, backup seluruh riwayat ke CSV, atau pindahkan catatan dari aplikasi lain.</p>
          <label className="data-label">Rentang laporan PDF</label>
          <div className="sort-toggle pdf-range" role="group" aria-label="Rentang laporan PDF">{[['this', 'Bulan ini'], ['last', 'Bulan lalu'], ['custom', 'Custom']].map(([value, label]) => <button key={value} type="button" className={pdfRange === value ? 'active' : ''} onClick={() => setPdfRange(value)}>{label}</button>)}</div>
          {pdfRange === 'custom' && <div className="pdf-custom"><input type="date" value={pdfCustom.start} max={today} onChange={(e) => setPdfCustom((cur) => ({ ...cur, start: e.target.value }))} aria-label="Tanggal mulai" /><span>s/d</span><input type="date" value={pdfCustom.end} max={today} onChange={(e) => setPdfCustom((cur) => ({ ...cur, end: e.target.value }))} aria-label="Tanggal selesai" /></div>}
          <div className="data-actions">
            <button className="clay-button brutal-button" onClick={handleExportPdf}>📄 Export PDF</button>
            <button className="clay-button brutal-button brutal-ghost" onClick={handleExportCsv}>🧾 Export CSV</button>
            <button className="clay-button brutal-button brutal-ghost" onClick={() => { setImportPreview(null); setImportSheetOpen(true); }}>📥 Import CSV</button>
          </div>
        </article>
      </section>
...
function ImportSheet({ preview, busy, onClose, onFile, onReset, onConfirm, money }) {
  ...
  return <div className="modal-backdrop clay-modal brutal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal clay-card brutal-sheet" role="dialog" aria-modal="true" aria-labelledby="import-title" onMouseDown={(event) => event.stopPropagation()}><button className="close-modal" onClick={onClose} aria-label="Tutup">×</button><p className="kicker">IMPORT CSV</p><h2 id="import-title">Pindahkan catatanmu</h2>
    {!preview ? <>
      <p className="import-hint">File CSV harus punya kolom <b>tanggal, tipe, kategori, judul, nominal</b> (tipe: income/expense). Kategori baru otomatis dibuat 📦 dan baris duplikat dilewati.</p>
      <label className="import-drop">
        <input type="file" accept=".csv,text/csv" onChange={onFile} />
        <span className="import-drop-inner"><b>📂 Pilih file CSV</b><small>maksimal 5MB</small></span>
      </label>
    </> : !result.headerOk ? <>
      <div className="import-errors"><p>❌ {result.invalid[0]?.reason}</p></div>
      <button className="clay-button brutal-button brutal-ghost import-confirm" onClick={onReset}>Coba file lain</button>
    </> : <>
      <p className="import-filename">{preview.fileName}</p>
      <div className="import-summary">
        {validCount > 0 && <span className="ok">✅ {validCount} siap masuk</span>}
        {result.duplicateCount > 0 && <span className="dup">🔁 {result.duplicateCount} duplikat dilewati</span>}
        {result.invalid.length > 0 && <span className="bad">⚠️ {result.invalid.length} baris error</span>}
        {!validCount && !result.invalid.length && <span className="dup">File kosong — nggak ada yang bisa diimpor</span>}
      </div>
      {result.invalid.length > 0 && <ul className="import-errors">{result.invalid.slice(0, 5).map((item) => <li key={item.row}><b>Baris {item.row}</b> — {item.reason}</li>)}{result.invalid.length > 5 && <li className="muted">+{result.invalid.length - 5} error lain…</li>}</ul>}
      {sample.length > 0 && <table className="import-sample"><thead><tr><th>Tanggal</th><th>Judul</th><th>Nominal</th></tr></thead><tbody>{sample.map((row, i) => <tr key={i}><td>{row.date}</td><td>{row.title}</td><td className={row.type === 'income' ? 'in' : 'out'}>{money.format(row.amount)}</td></tr>)}</tbody></table>}
      <button className="clay-button brutal-button primary-button import-confirm" disabled={busy || !validCount} onClick={onConfirm}>{busy ? 'Mengimpor…' : `Import ${validCount} transaksi`}</button>
    </>}
  </section></div>;
}
