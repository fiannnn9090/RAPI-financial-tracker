...
const initialData = { users: [], transactions: {}, goals: {}, achievements: {}, budgets: {}, badges: {}, badgeDefs: [], categories: {}, recurrings: {}, wallets: {}, challenges: {}, hasWallets: false, activeUserId: null };
...
async function loadData(userId) {
  const [probe, trx, goal, ach, bud, defs, owned, cats, recs, wallets, chl] = await Promise.all([
...
function Dashboard({ user, data, setData, autoGenerated = 0, fontScale = '1', onChangeFontScale, onChangeLang, lang = 'id', theme = 'system', onChangeTheme }) {
  ...
  }, [tab, showForm, budgetSheet, goalSheet, categorySheet, recurringSheet, walletSheet, challengeSheetOpen, importSheetOpen, levelUp, profileView]);
  ...
  function ProfileSubPage({ view, onBack }) {
    ...
    return <div className="profil-stack profile-sub">
      ...
      {view === 'pengaturan' && <article className="brutal-card settings-card">
        <nav className="settings-submenu" aria-label={t('prof.menu.akun')}>
          <button type="button" className="profile-menu-row" onClick={() => setProfileView('akun')}>
            <span className="row-icon" aria-hidden="true">🔑</span>
            <span className="row-label">{t('prof.menu.akun')}</span>
            <span className="row-chevron" aria-hidden="true">›</span>
          </button>
        </nav>
        <div className="setting-row" role="group" aria-label={t('settings.currency')}>
          <p className="setting-label"><span className="row-icon" aria-hidden="true">💱</span>{t('settings.currency')}</p>
        ...
        <div className="setting-row" role="group" aria-label={t('settings.fontSize')}>
          <p className="setting-label"><span className="row-icon" aria-hidden="true">🔠</span>{t('settings.fontSize')}</p>
        ...
        <div className="setting-row" role="group" aria-label={t('settings.language')}>
          <p className="setting-label"><span className="row-icon" aria-hidden="true">🌐</span>{t('settings.language')}</p>
          <div className="setting-seg">
        ...
        <div className="setting-row" role="group" aria-label={t('settings.theme')}>
          <p className="setting-label"><span className="row-icon" aria-hidden="true">🎨</span>{t('settings.theme')}</p>
          <div className="setting-seg">
      ...
      {view === 'akun' && <article className="brutal-card recurring-manage-card">
  ...
  return <main className="dashboard-page">
    <div className="dashboard app-frame" id="dashboard">
      ...
      {health.now.score != null && <ScoreCard health={health} onOpen={() => { setTab('profil'); setProfileView('saran'); }} />}
      ...
      <section className="challenge-section brutal-section">
        ...
        {completedThisWeek.map((row) => <article className="brutal-card challenge-card challenge-done" key={row.id}>
          ...
          <b className="challenge-chip">🏆 {t('ch.doneChip')}</b>
        </article>)}
        {challengeLive && !completedThisWeek.length && <article className={`brutal-card challenge-card ${challengeLive.progress.failed ? 'challenge-failed' : ''}`}>
      ...
      {tab === 'profil' && (profileView ? <ProfileSubPage view={profileView} onBack={() => setProfileView(profileView === 'akun' ? 'pengaturan' : null)} /> : <section className="profil-stack">
...
function ChallengeSheet({ activeCode = null, completedCodes = [], transactions = [], today, weekStart, onClose, onActivate }) {
  return <div className="modal-backdrop clay-modal brutal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal clay-card brutal-sheet" role="dialog" aria-modal="true" aria-labelledby="challenge-title" onMouseDown={(event) => event.stopPropagation()}><button className="close-modal" onClick={onClose} aria-label={t('common.close')}>×</button><p className="kicker">{t('ch.kicker')}</p><h2 id="challenge-title">{t('ch.pickTitle')}</h2><div className="challenge-options">
...
function Gauge({ score }) {
  ...
  const rot = clamped * 1.8 - 90;
  return <svg className="score-gauge" viewBox="0 0 120 72" aria-hidden="true">
    <path d={arc(Math.PI, (Math.PI * 2) / 3)} className="gauge-seg low" />
    <path d={arc((Math.PI * 2) / 3, Math.PI / 3)} className="gauge-seg mid" />
    <path d={arc(Math.PI / 3, 0)} className="gauge-seg high" />
    <line x1={CX} y1={CY} x2={CX} y2={26} transform={`rotate(${rot} ${CX} ${CY})`} className="gauge-needle" />
    <circle cx={CX} cy={CY} r="5.5" className="gauge-hub" />
  </svg>;
}
...
function ScoreCard({ health, onOpen }) {
  const { score, level } = health.now;
  return <button type="button" className="score-card clay-card brutal-card" aria-label={`${t('adv.score.kicker')} ${score}`} onClick={onOpen}>
    <span className="score-gauge-wrap"><Gauge score={score} /></span>
    <span className="score-card-body">
      <small className="kicker">{t('adv.score.kicker')}</small>
      <span className="score-num-line"><b className="score-num">{score}</b><small className="score-of">{t('adv.score.label')}</small></span>
      <span className={`score-chip ${level}`}>{t(`adv.score.${level}`)}</span>
