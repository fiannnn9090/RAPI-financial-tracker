    ...
    <section className="auth-panel" id="top">
      <div className="form-wrap">
        ...
        <form onSubmit={submit}>
          ...
          <label>Kata sandi<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minimal 4 karakter" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} /></label>
          {message && <p className="form-message" role="alert">{message}</p>}
          <button className="primary-button" type="submit">{mode === 'login' ? 'Masuk' : 'Buat akun'} <span>→</span></button>
        </form>
...
function Dashboard({ user, data, setData }) {
  ...
  return <main className="dashboard-page">
    <header className="topbar">
      ...
      <div className="account-menu"><span className="avatar">{user.username.slice(0, 1).toUpperCase()}</span><div><strong>{user.username}</strong><button onClick={logout}>Keluar</button></div></div>
    </header>
    <div className="dashboard" id="dashboard">
      ...
      <section className="playful-grid">
        <article className={`goal-card ${goalReached ? 'goal-reached' : ''}`}><div><p className="kicker">{goalReached ? 'WISHLIST TERCAPAI! 🎉' : 'TARGET TABUNGAN'}</p><h2>{goal ? goal.name : 'Punya wishlist?'}</h2>{goal ? <><div className="goal-progress"><span style={{ width: `${Math.min(100, Math.max(0, balance / goal.amount * 100))}%` }} /></div><p className="goal-caption"><strong>{rupiah.format(Math.max(0, balance))}</strong> dari {rupiah.format(goal.amount)}</p></> : <p className="goal-caption">Buat target kecil agar menabung terasa lebih seru.</p>}</div>{goalReached ? <button className="claim-goal" onClick={claimGoal}>Klaim badge 🏆</button> : <button onClick={setGoal}>{goal ? 'Ubah target' : '+ Buat target'}</button>}</article>
        <article className="badge-card"><div className="badge-heading"><div><p className="kicker">KOLEKSI BADGE</p><h2>Good job, bestie! ✨</h2></div><span>{badges.length}/4</span></div><div className="badges">{badges.length ? badges.map((badge) => <div className="badge" key={badge.title}><span>{badge.icon}</span><div><strong>{badge.title}</strong><small>{badge.note}</small></div></div>) : <p className="badge-empty">Catat transaksi pertamamu untuk membuka badge.</p>}</div></article>
      </section>
      <section className="insight-section"><div className="section-header"><div><h2>Money check-in</h2><p>Snapshot bulan ini, bestie 💫</p></div></div><div className="insight-grid"><article className="insight-card"><span>💡</span><div><strong>{topSpending ? `Paling banyak di ${CATEGORY_EMOJI[topSpending.category]} ${topSpending.category}` : 'Belum ada pengeluaran bulan ini'}</strong><p>{topSpending ? `Kamu sudah memakai ${rupiah.format(topSpending.amount)} untuk kategori ini.` : 'Mulai catat transaksi untuk melihat insight personal.'}</p></div></article><article className="chart-card"><div className="chart-title"><strong>Pengeluaran per kategori</strong><span>Bulan ini</span></div>{categorySummary.length ? <div className="chart-bars">{categorySummary.map((item) => <div className="chart-row" key={item.category}><span>{CATEGORY_EMOJI[item.category]}</span><div><div><strong>{item.category}</strong><b>{rupiah.format(item.amount)}</b></div><i><em style={{ width: `${item.amount / chartMax * 100}%` }} /></i></div></div>)}</div> : <p className="chart-empty">Grafik akan muncul setelah ada pengeluaran.</p>}</article></div></section>
      <section className="budget-section"><div className="section-header"><div><h2>Budget bulan ini</h2><p>Jaga pengeluaran tetap on track ✨</p></div><button className="budget-add" onClick={setBudget}>+ Atur budget</button></div>{budgetEntries.length ? <div className="budget-grid">{budgetEntries.map(([category, limit]) => { const spent = spendingFor(category); const ratio = spent / limit; const state = ratio >= 1 ? 'over' : ratio >= .8 ? 'near' : 'safe'; return <article className="budget-item" key={category}><div><span>{CATEGORY_EMOJI[category]}</span><strong>{category}</strong><button onClick={setBudget} aria-label={`Ubah budget ${category}`}>⋯</button></div><div className="budget-bar"><span className={state} style={{ width: `${Math.min(100, ratio * 100)}%` }} /></div><p><b>{rupiah.format(spent)}</b> / {rupiah.format(limit)} <em>{state === 'over' ? 'Kelebihan!' : state === 'near' ? 'Hampir habis' : 'Aman'}</em></p></article>; })}</div> : <div className="budget-empty"><span>🪄</span><div><strong>Belum ada budget</strong><p>Tentukan batas pengeluaran untuk kategori favoritmu.</p></div><button onClick={setBudget}>Buat budget</button></div>}</section>
