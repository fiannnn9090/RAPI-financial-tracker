...
export default function Home() {
  ...
  return activeUser
    ? <Dashboard user={activeUser} data={data} setData={setData} autoGenerated={autoGenerated} fontScale={fontScale} onChangeFontScale={changeFontScale} onChangeLang={changeLanguage} lang={lang} />
...
function Dashboard({ user, data, setData, autoGenerated = 0, fontScale = '1', onChangeFontScale, onChangeLang, lang = 'id' }) {
  ...
  const recap = useMemo(() => buildRecap({ transactions, period: recapPeriod, streak: user.streakCurrent ?? 0, money }), [transactions, recapPeriod, user.streakCurrent, money, lang]);
  ...
  return <main className="dashboard-page">
    <div className="dashboard app-frame" id="dashboard">
      ...
      <div className="profile-head"><span className="avatar big">{user.username.slice(0, 1).toUpperCase()}</span><div><strong>{user.username}</strong><small>{t('prof.lvTitle', { lvl: profInfo.level, title: titleForLevel(user.level ?? 1) })}</small></div></div>
      <article className="share-card clay-card brutal-card brutal-share"><p className="kicker">{t('share.kicker')}</p><h2>{t('share.title')}</h2><p className="share-note">{t('share.note')}</p><div className="card-actions"><button className="card-button clay-button brutal-button" onClick={downloadProfileCard}>{t('share.download')}</button>{canShareCard && <button className="card-button ghost-button clay-button brutal-button brutal-ghost" onClick={shareProfileCard}>{t('share.shareBtn')}</button>}</div></article>
      <section className="manage-settings brutal-section">
        <div className="section-header"><div><p className="kicker">{t('settings.kicker')}</p><h2>{t('settings.title')}</h2></div></div>
      ...
      <section className="manage-categories brutal-section">
        <div className="section-header"><div><p className="kicker">{t('cat.kicker')}</p><h2>{t('cat.title')}</h2></div><button className="clay-button brutal-button" onClick={() => setCategorySheet(true)}>{t('cat.add')}</button></div>
      ...
      <section className="manage-recurring brutal-section">
        <div className="section-header"><div><p className="kicker">{t('form.rec.kicker')}</p><h2>{t('recM.title')}</h2></div><button className="clay-button brutal-button" onClick={() => setRecurringSheet(true)}>{t('recM.add')}</button></div>
      ...
      <section className="manage-data brutal-section">
        <div className="section-header"><div><p className="kicker">{t('data.kicker')}</p><h2>{t('data.title')}</h2></div></div>
      ...
      {showReminderCard && <section className="manage-reminders brutal-section">
        <div className="section-header"><div><p className="kicker">{t('rem.kicker')}</p><h2>{t('rem.title')}</h2></div></div>
        <article className="brutal-card reminder-card">
          ...
          {reminderEnabled && <>
            ...
            <button type="button" className="clay-button brutal-button brutal-ghost reminder-test" onClick={() => fireTestPing()}>{t('rem.test')}</button>
          </>}
          {reminderMsg && <p className="form-message">{reminderMsg}</p>}
        </article>
      </section>}
      <section className="danger-zone clay-danger brutal-danger"><div><strong>{t('danger.title')}</strong><p>{t('danger.desc')}</p></div><button className="danger-button clay-button brutal-button brutal-danger-btn" onClick={deleteAccount}>{t('danger.title')}</button></section>
