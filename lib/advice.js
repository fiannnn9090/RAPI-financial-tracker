...
export function buildAdvice({
  ...
  if (dates.length < 5 || !dates[0]) return { items: [], checked: 0 };
  const firstDate = dates[0];
  const historyDays = Math.round((new Date(`${today}T00:00:00`) - new Date(`${firstDate}T00:00:00`)) / 86400000);
  if (historyDays < 14) return { items: [], checked: 0 };
  ...
  const ctx = {
    ...
    emoji,
    cur, prev, avgIncome, buffer, surplusAvg,
    dayOfMonth: d, daysInMonth, daysLeft: daysInMonth - d,
    microMax: 25000,
  };
