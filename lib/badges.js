...
/* Union def DB + fallback: DB menang per-kode, fallback mengisi celah.
   Seed kosong/parsal tidak boleh menyusutkan katalog (bug katalog 2/10). */
export function mergeBadgeDefs(dbDefs = []) {
  const dbCodes = new Set(dbDefs.map((def) => def.code));
  return [...FALLBACK_BADGE_DEFS.filter((def) => !dbCodes.has(def.code)), ...dbDefs];
}
