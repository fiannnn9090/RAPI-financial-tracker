...
export function badgeStats({ transactions, achievements, streakCurrent, level }) {
  return {
...
export function evaluateBadges(defs, stats, unlockedCodes) {
  return defs
    .map((def) => {
      ...
      return { ...def, current, target: config.target, progress: Math.min(1, current / config.target), unlocked };
    })
    .sort((a, b) => (
