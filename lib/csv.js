...
export function csvToTransactions(text, existingKeys = new Set()) {
  const rows = parseCsv(text);
  const invalid = [];
  const valid = [];
  let duplicateCount = 0;
  if (!rows.length) return { headerOk: false, valid, invalid: [{ row: 0, reason: 'File kosong' }], duplicateCount };
  ...
  if (missing.length) {
    return { headerOk: false, valid, invalid: [{ row: 0, reason: `Header harus memuat kolom: ${REQUIRED_COLUMNS.join(', ')}` }], duplicateCount };
  }
  rows.slice(1).forEach((cells, i) => {
    ...
    const problems = [];
    if (!isValidIsoDate(date)) problems.push('tanggal harus YYYY-MM-DD yang valid');
    if (type !== 'income' && type !== 'expense') problems.push('tipe harus income atau expense');
    if (!category || category.length > 60) problems.push('kategori kosong / terlalu panjang');
    if (!title || title.length > 120) problems.push('judul kosong / terlalu panjang');
    if (!/^\d+(\.\d{1,2})?$/.test(rawAmount) || !(Number(rawAmount) > 0)) problems.push('nominal harus angka polos > 0');
    if (problems.length) {
