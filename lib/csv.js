...
export function transactionsToCsv(transactions, walletNameOf = null) {
  const sorted = [...transactions].sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
  /* F4: kolom dompet opsional DI AKHIR — file lama tanpa kolom ini tetap
     bisa diimpor ulang (REQUIRED_COLUMNS tidak berubah). */
  const withWallet = typeof walletNameOf === 'function';
  const lines = [[...(withWallet ? ['tanggal', 'tipe', 'kategori', 'judul', 'nominal', 'dompet'] : ['tanggal', 'tipe', 'kategori', 'judul', 'nominal'])].join(',')];
  for (const tx of sorted) {
    const cells = [tx.date, tx.type, tx.category, tx.title, String(tx.amount)];
    if (withWallet) cells.push(walletNameOf(tx) ?? '');
...
export function csvToTransactions(text, existingKeys = new Set(), lang = 'id') {
  ...
  /* F4: kolom dompet opsional — nama dicocokkan ke daftar dompet user di pemanggil. */
  const walletCol = header.indexOf('dompet');
  rows.slice(1).forEach((cells, i) => {
    const rowNum = i + 2;
    const get = (col) => (idx[col] < cells.length ? cells[idx[col]].trim() : '');
    const getWallet = () => (walletCol >= 0 && walletCol < cells.length ? cells[walletCol].trim() : '');
    ...
    valid.push({ date, type, category, title, amount, ...(wallet ? { wallet } : {}) });
  });
