/* Gelar level bilingual — urutan & threshold sama, teks ikut lang. */
import { tl } from './i18n';
...
export function titleForLevel(level, lang = 'id') {
  if (level >= 10) return tl(lang, 'xp.title4');
  if (level >= 6) return tl(lang, 'xp.title3');
  if (level >= 3) return tl(lang, 'xp.title2');
  return tl(lang, 'xp.title1');
}
...
export function levelProgress(xp, lang = 'id') {
  ...
  return {
    level,
    title: titleForLevel(level, lang),
