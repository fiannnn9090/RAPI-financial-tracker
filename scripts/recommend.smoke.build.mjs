...
function summarize(transactions, { start, end }) {
  ...
  for (const tx2 of transactions) {
    if (!tx2 || tx2.date < start || tx2.date > end) continue;
    if (tx2.type === "income") {
      income += Number(tx2.amount) || 0;
      continue;
    }
    const amount = Number(tx2.amount) || 0;
    expense += amount;
    byCategory[tx2.category] = (byCategory[tx2.category] ?? 0) + amount;
  }
...
// lib/recommend.js
var SPLIT_IDEAL = { kebutuhan: 0.5, keinginan: 0.3, tabungan: 0.2 };
var STATUS_TOLERANCE = 0.1;
function buildBudgetRecommendation({
  ...
  for (const tx2 of transactions) {
    if (tx2.type !== "expense" || !tx2.date.startsWith(`${today.slice(0, 7)}`)) continue;
    const bucket = map[tx2.category] ?? "kebutuhan";
    actuals[bucket] += Number(tx2.amount) || 0;
  }
...
function hasExpense(transactions, today) {
  ...
  return transactions.some((tx2) => tx2.type === "expense" && tx2.date?.startsWith(prefix));
}

// scripts/recommend.smoke.mjs
...
var txs = [
  tx("2026-08-01", "income", "Gaji", 5e6),
  tx("2026-08-03", "expense", "Makan & Minum", 24e5),
  tx("2026-08-10", "expense", "Transportasi", 1e5),
  tx("2026-08-12", "expense", "Hiburan", 18e5),
  /* bulan lalu — tidak boleh ikut terhitung */
  tx("2026-07-15", "expense", "Nabung", 9e6)
];
var failures = 0;
...
var r = buildBudgetRecommendation({ transactions: txs, categories: CATS, today: TODAY });
check("enoughData true", r.enoughData);
check("incomeBase = income bulan ini", r.incomeBase === 5e6);
var byId = Object.fromEntries(r.buckets.map((b) => [b.id, b]));
...
check("ideal mengikuti 50/30/20", byId.kebutuhan.ideal === 25e5 && byId.keinginan.ideal === 15e5 && byId.tabungan.ideal === 1e6);
var r2 = buildBudgetRecommendation({
  transactions: [tx("2026-08-01", "income", "Gaji", 1e6), tx("2026-08-05", "expense", "KategoriHilang", 3e5)],
  categories: [],
  today: TODAY
});
check("kategori tak dikenal fallback kebutuhan", r2.buckets[0].id === "kebutuhan" && r2.buckets[0].actual === 3e5 && r2.buckets[0].status === "under");
var r3 = buildBudgetRecommendation({ transactions: [tx("2026-07-01", "expense", "Makan", 100)], categories: CATS, today: TODAY });
check("tanpa expense bulan ini \u2192 enoughData false", !r3.enoughData);
var again = buildBudgetRecommendation({ transactions: txs, categories: CATS, today: TODAY });
