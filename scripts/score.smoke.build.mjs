...
function scoreRecurring(stats, recurrings) {
  ...
  const burden = recurrings.filter((rule) => rule.type === "expense").reduce((sum, rule) => sum + monthlyEquivalent(rule), 0);
...
function buildScore({ transactions = [], budgets = {}, recurrings = [], today }) {
...
function previousMonthEnd(today) {
  const [y, m] = today.split("-").map(Number);
  const pm = shiftMonth(y, m, -1);
  return monthRange(pm.y, pm.m).end;
}

// scripts/score.smoke.mjs
...
function idealTx() {
  ...
  for (const [y, m] of [[2026, 5], [2026, 6], [2026, 7], [2026, 8]]) {
    tx.push(...monthTx(y, m, [
      [1, "income", "Gaji", 5e6],
      [3, "expense", "Makan & Minum", 12e5],
      [10, "expense", "Transportasi", 4e5],
      [20, "expense", "Hiburan", 4e5]
    ]));
  }
  return tx;
}
var idealBudgets = { "Makan & Minum": 15e5, Transportasi: 5e5, Hiburan: 5e5 };
var idealRecurrings = [{ type: "expense", title: "Streaming", amount: 15e4, frequency: "monthly" }];
function badTx() {
  const tx = [];
  for (const [y, m] of [[2026, 5], [2026, 6], [2026, 7], [2026, 8]]) {
    tx.push(...monthTx(y, m, [
      [1, "income", "Gaji", 5e6],
      [3, "expense", "Makan & Minum", 26e5],
      [10, "expense", "Transportasi", 9e5],
      [20, "expense", "Hiburan", 18e5]
    ]));
  }
  return tx;
}
var badBudgets = { "Makan & Minum": 1e6, Transportasi: 3e5, Hiburan: 5e5 };
var badRecurrings = [
  { type: "expense", title: "Kos", amount: 25e5, frequency: "monthly" },
  { type: "expense", title: "Cicilan", amount: 13e5, frequency: "monthly" }
];
var failures = 0;
...
var good = buildScore({ transactions: idealTx(), budgets: idealBudgets, recurrings: idealRecurrings, today: TODAY });
...
var bad = buildScore({ transactions: badTx(), budgets: badBudgets, recurrings: badRecurrings, today: TODAY });
check("buruk \u2192 skor <40", bad.score < 40, `score=${bad.score}`);
check("buruk \u2192 level perhatian", bad.level === "perhatian");
var noBudget = buildScore({ transactions: idealTx(), budgets: {}, recurrings: idealRecurrings, today: TODAY });
check("tanpa budget \u2192 basis 4", noBudget.basis === 4);
check("renormalisasi adil (\u226580)", noBudget.score >= 80, `score=${noBudget.score}`);
var sumMax = noBudget.components.reduce((s, c) => s + c.max, 0);
check("bobot renormalisasi = 75", sumMax === 75);
var thin = buildScore({ transactions: idealTx().slice(0, 3), budgets: {}, recurrings: [], today: TODAY });
...
var hist = buildScore({ transactions: idealTx(), budgets: idealBudgets, recurrings: idealRecurrings, today: previousMonthEnd(TODAY) });
check("skor bulan lalu terhitung", hist.enoughData && typeof hist.score === "number", `prev=${hist.score}`);
var again = buildScore({ transactions: idealTx(), budgets: idealBudgets, recurrings: idealRecurrings, today: TODAY });
